...
export function ComparisonView({ comparison, slug }: { comparison: DirectoryEntry; slug: string }) {
  ...
  // Phase 4c (2026-05-19): comparisonTitle still falls back to URL-derived
  // `{left} vs {right}` because both sides come from the parsed slug (operator-
  // visible). introText drops the literal "solve similar problems..." fallback
  // — section now hides when CC has nothing.
  const comparisonTitle = comparison.heroTitle || comparison.title || `${left} vs ${right}`;
  const introText = comparison.heroDescription || comparison.excerpt || "";
  const contentHeadings = portableTextHeadings(comparison.body, comparison.bodyHtml);
  // CC↔Live truth fix (2026-05-19, Phase 4c): drop hardcoded tableRows fallback.
  // Was injecting 4 fake-price rows ($49/$19/$9 unit pricing) + "Unified Inbox",
  // "Direct Booking Site", "Renovation Calculator" comparison whenever CC table
  // was empty — entirely fabricated content the operator could not see.
  const tableRows = comparison.comparisonTableRows?.length ? comparison.comparisonTableRows : [];
  // CC↔Live truth fix (2026-05-19, Phase 4c): drop generic 3-FAQ fallback
  // ("Does {left} work in Europe?", "Is {right} good for scaling?", "Can I
  // migrate my data?"). Mirrors Phase 2 glossary fix.
