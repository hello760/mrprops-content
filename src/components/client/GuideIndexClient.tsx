            ...
            <div className="space-y-6 animate-in slide-in-from-left-5 duration-700">
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                ...
                <div className="flex items-center text-sm font-medium text-muted-foreground ml-2">
                  <span className="h-1 w-1 rounded-full bg-primary mx-3" /> {featuredGuide?.readTime}
                </div>
              </div>
            </div>
            ...
            {GUIDE_CATEGORIES.map((cat) => (
              <button
                ...
                className={cn(
                  ...
                  selectedCategory === cat ? "bg-foreground text-background" : "hover:bg-secondary text-muted-foreground",
                )}
              >
                {cat}
              </button>
      ...
      <div className="container mx-auto px-4 py-16">
        <div className="bg-primary text-primary-foreground rounded-3xl p-8 md:p-12 mb-16 flex flex-col md:flex-row items-center justify-between gap-8 shadow-xl shadow-primary/20 relative overflow-hidden">
          ...
          <div className="relative z-10 space-y-4 max-w-xl text-center md:text-left">
            <h3 className="font-display text-2xl md:text-3xl font-bold">Join 15,000+ Smart Hosts</h3>
            <p className="text-primary-foreground/80 text-lg">
              Get weekly tips on automation, tax loopholes, and market trends delivered to your inbox.
            </p>
          </div>
          <div className="relative z-10 w-full md:w-auto flex flex-col sm:flex-row gap-3">
            ...
            />
            <Button variant="secondary" size="lg" className="h-12 font-bold px-8 rounded-xl shadow-lg">
              Subscribe
            </Button>
          </div>
        </div>
              ...
              <Link key={post.id} href={`/guides/${post.slug}`}>
                <div className="group cursor-pointer flex flex-col h-full bg-card border border-border rounded-2xl overflow-hidden hover:shadow-2xl hover:shadow-primary/5 hover:-translate-y-1 transition-all duration-300">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                    ...
                    <p className="text-muted-foreground text-sm mb-6 line-clamp-3 flex-1 leading-relaxed">{post.excerpt}</p>
        ...
        {filteredPosts.length === 0 && (
          <div className="rounded-3xl border border-border bg-card px-8 py-16 text-center mb-16">
            <h3 className="font-display text-2xl font-bold mb-3">No guides matched that search</h3>
            <p className="text-muted-foreground mb-6">Try a broader keyword or switch back to all categories.</p>
            <Button
              variant="outline"
              onClick={() => {
                setSearchTerm("");
          ...
          <div className="flex justify-center items-center gap-2">
            ...
            {Array.from({ length: totalPages }).map((_, i) => (
              <Button
                ...
                className={`rounded-full h-10 w-10 ${page === i + 1 ? "bg-primary text-white hover:bg-primary/90 shadow-lg shadow-primary/25" : "border-border hover:bg-primary/10 hover:text-primary hover:border-primary/50"}`}
                onClick={() => setPage(i + 1)}
              >
                {i + 1}
              </Button>
