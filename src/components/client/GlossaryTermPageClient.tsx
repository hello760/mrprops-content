      ...
      <div className="container mx-auto px-4 max-w-screen-xl py-12">
        <div className="grid lg:grid-cols-[1fr_350px] gap-12 items-start">
          ...
          <aside className="space-y-8 lg:sticky lg:top-24 h-fit">
            ...
            {/* FIX-025 (PF-25): relatedTerms upgraded to object shape { term, relationship }
                per PDF glossary GLOBAL 8. Renderer accepts both legacy string[] and new
                object[] shapes — normalize to { term, relationship? } for display. */}
            <div className="bg-card border border-border p-6 rounded-xl shadow-sm">
                  ...
                  return (
                    <li key={`${termStr}-${i}`}>
                      <Link href={toGlossaryHref(termStr)} className="group block text-foreground hover:text-primary transition-colors cursor-pointer text-sm py-1">
                        ...
                        {relationshipStr ? (
                          <div className="mt-1 text-xs text-muted-foreground leading-snug">{relationshipStr}</div>
                        ) : null}
                      </Link>
                    </li>
                  );
