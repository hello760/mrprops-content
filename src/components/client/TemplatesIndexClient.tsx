...
function templateFormat(page: DirectoryEntry & { category: string }) {
  const category = titleCaseCategory(page.category);
  if (category === "Legal") return "Word, PDF";
  if (category === "Guest Experience") return "Notion, PDF";
  if (category === "Marketing") return "PDF";
  return "Excel, PDF";
}

function templateDownloads(index: number) {
  const values = ["2.4k", "5.1k", "3.2k", "1.8k", "4.5k", "6.2k", "1.2k", "900+", "2.1k", "3.8k"];
  ...
  return (
    <div className="min-h-screen bg-background pb-20 pt-4 md:pt-8">
      <div className="container mx-auto px-4 max-w-screen-xl">
        <div className="flex items-center text-sm text-muted-foreground gap-2 mb-6 overflow-x-auto whitespace-nowrap">
      ...
      <div className="bg-primary/5 border-b border-border mb-12 rounded-3xl mx-4 overflow-hidden">
        <div className="container mx-auto px-4 py-12 md:py-16 text-center max-w-4xl">
          <Badge className="mb-4 bg-primary/10 text-primary hover:bg-primary/20 border-primary/20">Free Resources</Badge>
          <h1 className="font-display text-3xl md:text-6xl font-bold mb-6">
            Everything You Need to Run <br className="hidden md:block" />a Better Short-Term Rental
          </h1>
          <p className="text-lg md:text-xl text-muted-foreground mb-8 max-w-2xl mx-auto">
            Download our battle-tested templates, checklists, and calculators. Used by over 10,000+ hosts to save time and increase revenue.
          </p>
      ...
      <div className="container mx-auto px-4 max-w-screen-xl">
        <div className="flex flex-wrap items-center gap-2 mb-10 pb-4 border-b border-border overflow-x-auto whitespace-nowrap">
            ...
            <Link key={template.id} href={template.href}>
              <div className="group bg-card border border-border rounded-xl overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300 cursor-pointer h-full flex flex-col">
                ...
                <div className="p-5 flex flex-col flex-grow">
                  <div className="text-xs font-bold text-primary mb-2 uppercase tracking-wide">{titleCaseCategory(template.category)}</div>
                  <h3 className="font-bold text-lg mb-2 group-hover:text-primary transition-colors line-clamp-2">{template.title}</h3>

                  <div className="mt-auto pt-4 flex items-center justify-between text-xs text-muted-foreground">
                    <div className="flex items-center gap-1">
                      <FileText className="h-3 w-3" /> {templateFormat(template)}
                    </div>
                    <div className="flex items-center gap-1">
                      <Download className="h-3 w-3" /> {templateDownloads(index)}
                    </div>
                  </div>
                </div>
              </div>
