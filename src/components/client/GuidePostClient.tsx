...
import { CalendarDays, ChevronRight, Clock, Home } from "lucide-react";
import { cn } from "@/lib/utils";
import type { DirectoryEntry } from "@/lib/content-pages";
import { PortableTextContent } from "@/components/content/PortableTextContent";

export function GuidePostClient({
  guide,
  relatedGuides,
}: {
  guide: DirectoryEntry & { date?: string; readTime?: string };
  relatedGuides: Array<DirectoryEntry & { date?: string; readTime?: string }>;
}) {
  ...
  const sections = useMemo(() => {
    return (guide.body || [])
      ...
      .map((block) => {
        ...
        return { id: text.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, ""), label: text };
      });
  }, [guide.body]);
    ...
    <article className="min-h-screen bg-background pb-20 pt-20">
      ...
      <header className="container mx-auto px-4 max-w-4xl text-center mb-16 animate-in slide-in-from-bottom-5 duration-700">
        <div className="flex items-center justify-center gap-3 mb-6">
          ...
          <span className="text-muted-foreground text-sm font-medium">Updated {guide.date || guide.updated}</span>
        </div>
        <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold leading-[1.1] tracking-tight text-foreground mb-8">
          {guide.title}
        </h1>

        <div className="flex flex-col sm:flex-row items-center justify-center gap-6 text-muted-foreground">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 border border-border rounded-full overflow-hidden bg-primary text-white flex items-center justify-center text-sm font-bold">MP</div>
            <div className="text-left">
              <div className="font-bold text-foreground text-sm leading-none">Mr. Props Team</div>
              <div className="text-xs">Operational Experts</div>
            </div>
          </div>
          <div className="hidden sm:block w-px h-8 bg-border" />
          <div className="flex items-center gap-4 text-sm font-medium">
            <div className="flex items-center gap-1.5"><CalendarDays className="h-4 w-4" /> {guide.date || guide.updated}</div>
            <div className="flex items-center gap-1.5"><Clock className="h-4 w-4" /> {guide.readTime || "8 min read"}</div>
          </div>
        </div>
      </header>
          ...
          <nav className="space-y-1 border-l border-border pl-4">
            {sections.map((item) => (
              <button
                ...
                className={cn(
                  "block text-sm py-1.5 transition-colors text-left w-full hover:text-primary",
                  activeSection === item.id ? "text-primary font-bold -ml-[17px] pl-[13px] border-l-4 border-primary" : "text-muted-foreground",
                )}
              >
                {item.label}
              </button>
            ...
            <div className="lg:hidden mb-12 bg-secondary/20 rounded-xl p-6 border border-border">
              ...
              <ul className="space-y-3 list-none pl-0 my-0">
                {sections.map((item, index) => (
                  <li key={item.id}>
                    <button onClick={() => scrollToSection(item.id)} className="text-primary font-medium hover:underline text-base pl-0 flex items-center gap-2">
                      <span className="text-muted-foreground text-sm font-mono">{String(index + 1).padStart(2, "0")}.</span> {item.label}
                    </button>
                  </li>
                ))}
              </ul>
              ...
              {relatedGuides.map((item) => (
                <li key={item.slug} className="group cursor-pointer">
                  <Link href={`/guides/${item.slug}`}>
                    <div className="text-xs font-bold text-primary mb-1">{item.heroBadge || "Guide"}</div>
                    <div className="font-bold text-sm group-hover:text-primary transition-colors leading-snug">
                      {item.title}
                    </div>
                  </Link>
                </li>
