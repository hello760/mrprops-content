...
import { AlertCircle, Check, X, Minus, ArrowRight } from "lucide-react";
...
function renderTableCell(value: string | boolean, isPrimary = false) {
...
export default async function AlternativePage({ params }: { params: Promise<{ competitor: string }> }) {
  ...
  const structuredData = buildStructuredData(createBreadcrumbSchema([{ name: "Home", path: "/" }, { name: "Alternatives", path: "/alternatives" }, { name: alternative.title }]), createFaqSchema(faqs));
    ...
    <>
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto px-4 py-24 max-w-screen-xl">
          ...
          {/* Comparison Table — rendered when comparisonTableRows exist */}
            ...
            <div id="comparison-table" className="mb-24 scroll-mt-24">
              <div className="mb-8 text-center">
                ...
                <h2 className="mt-2 font-display text-3xl md:text-4xl font-bold">
                  {competitorName} vs Mr. Props
                </h2>
                <p className="mt-3 text-muted-foreground max-w-2xl mx-auto">
                  See exactly where {competitorName} falls short and how Mr. Props closes the gap.
                </p>
              </div>
                ...
                <table className="w-full min-w-[640px] border-collapse">
                  <thead>
                    <tr>
                      ...
                      <th className="w-1/3 border-b border-border bg-muted/20 p-6 text-center text-lg font-bold text-muted-foreground">
                        {competitorName}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    {comparisonTableRows.map((row, i) => (
                      <tr key={i} className="border-b border-border transition-colors hover:bg-muted/30 last:border-0">
                        ...
                        <td className="bg-primary/5 p-6 text-center text-base font-semibold text-foreground">
                          {renderTableCell(row.primaryValue ?? "", true)}
                        </td>
                        <td className="border-l border-border/50 bg-muted/5 p-6 text-center text-muted-foreground">
                          {renderTableCell(row.secondaryValue ?? "")}
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            <div className="grid lg:grid-cols-2 gap-12 items-center relative z-10">
              <div className="space-y-6">
                <h2 className="font-display text-3xl md:text-5xl font-bold">
                  {alternative.migrationTitle || "Switching is instant."}
                </h2>
                <p className="text-xl text-background/80 leading-relaxed">
                  {alternative.migrationDescription || `Don't worry about your data. Our 1-click migration tool imports your listings, reviews, and future bookings from ${competitorName} in seconds.`}
                </p>
              </div>
              <div className="bg-white/10 backdrop-blur-md rounded-2xl p-6 border border-white/10 shadow-2xl">
                ...
                <div className="mt-6 text-center">
                  <Button size="lg" className="bg-white text-black hover:bg-gray-200 font-bold rounded-full w-full">
                    {alternative.migrationButtonLabel || "Start My Import"}
                  </Button>
                </div>
              </div>
            </div>
